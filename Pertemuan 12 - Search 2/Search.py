...
class Search:
  ...
  def InputData():
    while True:
      ...
      if (data is None) or (data is ''):
        ...
        i = 0
        while i < len(data):
          data[i] = int(data[i]); i+=1
        break
    Search.__data = data.copy()
    Search.Selection(Search.__data)
    Search.ChooseAlgorithm(Search.__data)
    Search.TryAgain()
  ...
  def ChooseAlgorithm(data):
    ...
    while True:
      while True:
        index = input('\nSilahkan pilih algoritma yang digunakan:\n  [1] Linear Algorithm\n  [2] Binary Algorithm\n  Pilihanmu\t? ')
        if index.isnumeric():
          index = int(index); break
      if index == 1:
        Search.Linear(data); break
      if index == 2:
        Search.Binary(data); break
  ...
  def TryAgain():
    while True:
      ...
      if confirm == 'n' or confirm is '' or confirm is 'N':
        ...
        Search.ChooseAlgorithm(Search.__data); break
  ...
  def Linear(data):
    ...
    while True:
      ...
      if data[index] == key:
        print("  Angka",key,"Ditemukan."); break
      elif index > len(data):
        print("  Angka",key,"Tidak ditemukan."); break
      index+=1
      # sleep(1)
    timeEnd = time()
    print((timeEnd - timeStart), "detik.")

  # Menggunakan Algoritma Binary Search
  @staticmethod
  def Binary(data):
    key = ''; found = False; index = 0; first = 0; last = len(data)-1; rudand = index; start = 0
    print("\n-- Menggunakan Algoritma Binary Search --\n")
    while not key.isnumeric():
      key = input("Masukkan Angka yg dicari\t: ")
    key = int(key)
    ...
    while not found:
      ...
      print("Mencari pada index ke-"+str(index))
      if data[index] == key:
        found = True; print("  Angka",key,"Ditemukan.")
      elif data[index] < key:
        first = (int)(index)+1; last = last
      else:
        first = first; last = index-1
      if index == rudand:
        found = True; print("  Angka",key,"Tidak ditemukan.")
      # sleep(1)
    timeEnd = time()
    print((timeEnd - timeStart), "detik.")
